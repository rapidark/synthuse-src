...
import test2.Api.WinDefEx.LVITEM_VISTA;
...
public class Api {
	...
	public static void main(String[] args) {
		HWND hwnd = User32Ex.instance.FindWindow("#32770", "Windows 任务管理器");
        hwnd = User32Ex.instance.FindWindowEx(hwnd, 0, "#32770", null);
        hwnd = User32Ex.instance.FindWindowEx(hwnd, 0, "SysListView32", null);//进程界面窗口的句柄,通过SPY获取
//        headerhwnd = User32Ex.instance.SendMessage(hwnd, LVM_GETHEADER, 0, 0);//listview的列头句柄
        
		HWND listViewHwnd = hwnd;
		System.out.println(listViewHwnd);
		/*
		HANDLE hProcess = OpenProcess(PROCESS_VM_WRITE | PROCESS_VM_OPERATION, FALSE, 0x0000c130);
		LPVOID epLvi = VirtualAllocEx(hProcess, NULL, 4096, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		LVITEM lvi;
		...
		*/
	    ...
	    lvitem.pszText = new Memory(256);
//		lvitem.stateMask = LVIS_FOCUSED | LVIS_SELECTED;
//		lvitem.state = LVIS_FOCUSED | LVIS_SELECTED;
		IntByReference bytesWritten = new IntByReference();
		Api.Kernel32Ex.instance.WriteProcessMemory(new HANDLE(process), epLvi, lvitem.getPointer(), lvitem.size(),bytesWritten);
		LRESULT isSuccess = Api.User32Ex.instance.SendMessage(listViewHwnd, 4171, new WPARAM(0), lvitem);
		System.out.println("isSuccess:" + isSuccess);
//		Api.User32Ex.instance.SendMessage(listViewHwnd, LVM_SETITEMSTATE, new WPARAM(0), lvitem);
		
		byte[] result = new byte[256];
		ByteBuffer bufDest = ByteBuffer.allocateDirect(256);
//		bufDest.put(result);
//		System.out.println(Arrays.toString(bufDest.array()));
//        bufDest.put(new byte[]{0,1,2,3});
//        ByteBuffer bufSrc = ByteBuffer.allocateDirect(4);
//        bufSrc.put(new byte[]{5,10,15,20});
        Pointer ptrSrc = Native.getDirectBufferPointer(bufDest);
        
		Api.Kernel32Ex.instance.ReadProcessMemory(new HANDLE(process), lvitem.pszText, ptrSrc, 256,bytesWritten);
		System.out.println(Arrays.toString(bufDest.array()));
		Api.Kernel32Ex.instance.VirtualFreeEx(new HANDLE(process), epLvi, new SIZE_T(4096), new DWORD(MEM_DECOMMIT | MEM_RELEASE));
		Api.Kernel32Ex.instance.CloseHandle(new HANDLE(process));
	}
