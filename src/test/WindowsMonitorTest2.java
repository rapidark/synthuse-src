 ...
 * @version V1.0
 */
public class WindowsMonitorTest2 {
...
//
    private int PROCESS_VM_OPERATION = 0x0008;//允许函数VirtualProtectEx使用此句柄修改进程的虚拟内存
    private int PROCESS_VM_READ = 0x0010;//允许函数访问权限
    private int PROCESS_VM_WRITE = 0x0020;//允许函数写入权限
//
    private int MEM_COMMIT = 0x1000;//为特定的页面区域分配内存中或磁盘的页面文件中的物理存储
    private int MEM_RELEASE = 0x8000;
    private int MEM_RESERVE = 0x2000;//保留进程的虚拟地址空间,而不分配任何物理存储
//
    private int PAGE_READWRITE = 4;
...
//    }
    ...
    {
        return Api.User32Ex.instance.SendMessage(handle, Api.LVM_GETITEMCOUNT, new WPARAM(0), new LPARAM(0)).longValue();
    }
    /// <summary>  
    /// LV列表总列数
    /// </summary>
    ...
    {
        return Api.User32Ex.instance.SendMessage(handle, HDM_GETITEMCOUNT, new WPARAM(0), new LPARAM(0)).longValue();
    }
...
//        hwnd = FindWindow("#32770", "Windows 任务管理器");
        ...
        headerhwnd = User32Ex.instance.SendMessage(hwnd, LVM_GETHEADER, new WPARAM(0), new LPARAM(0)).intValue();//listview的列头句柄
//
...
//
       String text =  test2(hwnd, 1);
       System.out.println("text:" + text);
        
//        tempStr = GetListViewItemValue((int)rows, (int)cols);//将要读取的其他程序中的ListView控件中的文本内容保存到二维数组中
//
...
//        }
    ...
    public String test2(HWND hWnd, int itemIdx) {
		...
		Pointer lngVarPtr1 = null;
		Pointer lngMemVar1 = null;
		Pointer lngVarPtr2 = null;
		Pointer lngMemVar2 = null;
		Pointer lviVarPtr = null;
		Pointer lviVar = null;
		int lngMemLen1;
		int lngMemLen2;
    	    ...
    	    lvi.iItem = itemIdx;
    	    lvi.iSubItem = 0;
    	    lvi.mask = new UINT(LVIF_TEXT);
//    	    lvi.pszText = lngMemVar1;       
    	    lngMemVar1 = lvi.pszText;
    	    ...
    	    result = user32.SendMessage (hWnd, LVM_GETITEMTEXT, 0, lngMemVar2);

    	   lngVarPtr1 = new Memory(strSize + 1);
    	   result = kernel32.ReadProcessMemory(lngProcHandle, lngMemVar1, lngVarPtr1, lngMemLen1, byteIO);

    	    result = kernel32.VirtualFreeEx (lngProcHandle, lngMemVar1, 0, Api.MEM_RELEASE);
    	    result = kernel32.VirtualFreeEx (lngProcHandle, lngMemVar2, 0, Api.MEM_RELEASE);        
    	     kernel32.CloseHandle(lngProcHandle);
    ...
    private String[][] GetListViewItemValue(int rows, int cols) {
//    	GetListViewItmeValue
//    	GetListViewItemValue
...
//    for item_index in range(num_items):
    	for (int i = 0; i < rows; i++) {
			for (int j=0; j<cols;j++) {
            	...
            	item.pszText = new Pointer(pBuffer);
//            	System.out.println("pBuffer.getPointer():" + Pointer.nativeValue(pBuffer.getPointer()));
				IntByReference vNumberOfBytesRead = new IntByReference();
//				System.out.println(vNumberOfBytesRead.getValue());
            	Kernel32Ex.instance.WriteProcessMemory(process, pLVI, (int)Pointer.nativeValue(item.getPointer()), size, vNumberOfBytesRead);
//            	System.out.println(vNumberOfBytesRead.getValue());
//            	  User32Ex.instance.SendMessage(hwnd, LVM_GETITEMTEXT, new WPARAM(i),pLVI);
            	  
            	  int strSize = 255;
                  Memory lngVarPtr1 = new Memory(4096);
//                  System.out.println(mem.getString(0));
//                  try {
//                	  System.out.println(new String(mem.getByteArray(0, 1024), "gbk"));
//                	  System.out.println(new String(mem.getByteArray(0, 1024), "utf-8"));
...
//                  System.out.println(vNumberOfBytesRead.getValue());
//                  System.out.println("pBuffer.getPointer():" + Pointer.nativeValue(pBuffer.getPointer()));
                  Kernel32Ex.instance.ReadProcessMemory(process,  pBuffer, lngVarPtr1, 4096, vNumberOfBytesRead);
   //
                  System.out.println("==" + lngVarPtr1.getWideString(0));
			}
...
//    private String[][] __GetListViewItmeValue(int rows, int cols)
//    {
...
//            	IntByReference vNumberOfBytesRead = new IntByReference();
//            	Kernel32Ex.instance.WriteProcessMemory(process, pLVI, item.getPointer(), item.size(), vNumberOfBytesRead);
////
////                //发送LVM_GETITEMW消息给hwnd,将返回的结果写入pointer指向的内存空间
//               User32Ex.instance.SendMessage(hwnd, LVM_GETITEMW, new WPARAM(i),new LPARAM( pLVI.getValue()));
////
...
////               Kernel32Ex.instance.ReadProcessMemory(process.getPointer(),  Pointer.nativeValue(item.pszText), lngVarPtr1, strSize + 1, vNumberOfBytesRead);
////
...
//    }
    ...
    public static void main(String[] args) {
		new WindowsMonitorTest2().button1_Click_1();
	}
